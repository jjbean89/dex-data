...
2. **Count history** — their 2h-sampled long/short position-count series per coin (back toward April 2025) imported into `positioning_snapshots` with `source: "hypertracker"`, so your change-over-time chart starts long before this service existed. Live rows are never overwritten; consumers can distinguish sources via the `source` field (backfilled rows have no per-coin size breakdown or `tradersTracked`).
