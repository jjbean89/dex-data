...
- Positions are maintained from fills with the `[buyer, seller]` convention (verified empirically: 29/33 exact matches to 1e-9 against clearinghouseState, remainder explained by in-flight fills). **Entry prices are maintained live from the tape too** (open sets, adds blend size-weighted, reduces hold, flips reset — the same average-entry math Hyperliquid uses), so `entries` coverage approaches 100% even on fast-churning books; positions predating the tracker keep their verified entry until re-checked. WebSocket gaps trigger automatic re-baselining of recently active wallets, and a rolling re-verify sweep re-checks the longest-unverified wallets — drift self-heals within hours.
